...
const BuyNow = () => {
   ...
   const watchId = cart[0]


   useEffect(() => {
      if (cart.length !== 0) {
         fetch(`https://mighty-retreat-45944.herokuapp.com/products/${watchId}`)
            .then(res => res.json())
            .then(data => {
               setBike(data);
            })
   ...
   const order = { user: user.email, name: bike.bikeName, img: bike.img, status: 'pending' }
